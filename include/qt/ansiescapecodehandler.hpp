...
inline AnsiEscapeCodeHandler::AnsiEscapeCodeHandler() : m_scrollBar(nullptr){}

inline AnsiEscapeCodeHandler::AnsiEscapeCodeHandler(QTextCursor textCursor, QScrollBar *scrollBar) : m_textCursor(textCursor), m_scrollBar(scrollBar){}

inline AnsiEscapeCodeHandler &AnsiEscapeCodeHandler::operator=(AnsiEscapeCodeHandler &&move)
{
    m_previousFormatClosed = move.m_previousFormatClosed;
    m_waitingForTerminator = move.m_waitingForTerminator;
    m_alternateTerminator  = move.m_alternateTerminator;
    m_previousFormat       = move.m_previousFormat;
    m_pendingText          = move.m_pendingText;
    m_textCursor           = move.m_textCursor;
    m_scrollBar            = move.m_scrollBar;
    return *this;
}

inline void AnsiEscapeCodeHandler::appendAnsiText(const QString &text)
{
    ...
    if(m_scrollBar && at_bottom)
        m_scrollBar->setValue(m_scrollBar->maximum());
}



inline QList<FormattedText> AnsiEscapeCodeHandler::parseText(const FormattedText &input)
{
...
inline void AnsiEscapeCodeHandler::endFormatScope()
{
    m_previousFormatClosed = true;
}


inline void AnsiEscapeCodeHandler::setFormatScope(const QTextCharFormat &charFormat)
{
